...
import base64, time
import unicodedata
AddonID = 'plugin.video.jedi-streams'
ADDON=xbmcaddon.Addon(id='plugin.video.jedi-streams')
__icon__        = ADDON.getAddonInfo('icon')
__self__        = xbmcaddon.Addon(id='plugin.video.jedi-streams')
...
def correctPVR():
	...
	flawiptv = xbmcaddon.Addon('plugin.video.jedi-streams')
	...
	dialog.ok("Jedi Streams",'IPTV Simple Client set-up is complete.',' ','Please allow it time to load the EPG to avoid issues.')
...
def speed():
	xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.jedi-streams/speedtest.py")')
