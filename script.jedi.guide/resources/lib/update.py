...
def check_update():
	text_file = xbmc.translatePath('special://home/addons/script.jedi.guide/resources/sfhash.txt')
	HTML = OPEN_URL('https://github.com/Jedi GuideTV/addons/blob/master/Jedi Guide.zip')
	regex = re.compile('Jedi GuideTV/addons/commit/(.+?)" class="message').findall(HTML)
	for commit in regex:
		text = commit
		...
		if text == text_file_contents:
			...
			text_file.close()
			print commit
			xbmc.executebuiltin('RunScript(special://home/addons/script.jedi.guide/resources/lib/updateSF.py)')
