...
class Database(object):
    ...
    def loadOptional(self,force):
        ...
        if (ADDON.getSetting('addons.ini.subscriptions') == "true"):
            file_name = 'special://profile/addon_data/script.jedi.guide/subscriptions.ini'
            ...
            for (name,sub) in name_sub:
                ...
                f.close
                if not data:
                    d.notification("Jedi Guide","%s - %s" % (name,sub), xbmcgui.NOTIFICATION_ERROR)
        ...
        path = "special://profile/addon_data/script.jedi.guide/addons.ini"
        if not xbmcvfs.exists(path):
            f = xbmcvfs.File(path,"w")
            f.close()

        addons_ini = "special://profile/addon_data/script.jedi.guide/addons.ini"
        ...
        if ADDON.getSetting('addons.ini.enabled') == 'true':
            ...
            if data:
                ...
                if encode and enckey:
                    ...
                    f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/addons.aes.ini','wb')
                    f.write(ddata)
                    f.close()
                elif enckey:
                    import pyaes,base64
                ...
                xbmcvfs.File('special://profile/addon_data/script.jedi.guide/addons.ini','wb').write(data)

        if (ADDON.getSetting('addons.ini.subscriptions') == "true") or (ADDON.getSetting('addons.ini.overwrite') == "1"):
            streams = {}
            streams["script.jedi.guide"] = {}
            ...
            if (ADDON.getSetting('addons.ini.subscriptions') == "true"):
                for name in subscription_streams:
                    if name:
                        streams["script.jedi.guide"][name] = subscription_streams[name]
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.jedi.guide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.jedi.guide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.jedi.guide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        lock = 'special://profile/addon_data/script.jedi.guide/db.lock'
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.jedi.guide] Updating caches...', xbmc.LOGDEBUG)
            ...
            if getData == True:
                xbmcvfs.delete('special://profile/addon_data/script.jedi.guide/category_count.ini')
    ...
    def exportChannelList(self):
        ...
        f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/channels.ini','wb')
    ...
    def exportChannelIdList(self):
        ...
        f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/channel_id_title.ini','wb')
    ...
    def _createTables(self):
        ...
        #except sqlite3.OperationalError, ex:
            ...
            xbmc.log("(script.jedi.guide) %s" % detail, xbmc.LOGERROR)
            dialog = xbmcgui.Dialog()
            dialog.notification('script.jedi.guide', 'database exception %s' % detail, xbmcgui.NOTIFICATION_ERROR , 5000)
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.jedi.guide'))
    KEY = 'xmltv'
    INI_TYPE_FILE = 0
    INI_TYPE_URL = 1
    INI_FILE = 'addons.ini'
    ...
    def __init__(self, addon, force):
        ...
        if self.xmltvType == XMLTVSource.XMLTV_SOURCE_FILE:
            ...
            if os.path.exists(customFile):
                ...
                xbmc.log('[script.jedi.guide] Use local file: %s' % customFile, xbmc.LOGDEBUG)
                self.xmltvFile = customFile
            else:
                # Probably a remote file
                xbmc.log('[script.jedi.guide] Use remote file: %s' % customFile, xbmc.LOGDEBUG)
    ...
    def parseXMLTVDate(self, origDateString):
        ...
        try:
            ...
            xbmc.log('[script.jedi.guide] strptime error with this date: %s' % dateString, xbmc.LOGDEBUG)
    ...
    def parseXMLTV(self, context, f, size, logoFolder, progress_callback):
        ...
        data = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/channel_id_shortcut.ini','rb').read()
        id_shortcuts = {}
        if data:
            lines = data.splitlines()
            for line in lines:
        ...
        if ADDON.getSetting('update.progress') == 'true':
            ...
            d.create('Jedi Guide', "parsing xmltv")
        category_count = {}
        ...
        if ADDON.getSetting('update.progress') == 'true':
            ...
            d.close()
        f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/category_count.ini',"wb")
...
class TVGUKNow2Source(Source):
    ...
    def getDataFromExternal2(self, date, ch_list, progress_callback=None):
        ...
        if ADDON.getSetting('update.progress') == 'true':
            d = xbmcgui.DialogProgressBG()
            d.create('Jedi Guide', "grabbing tvguide.co.uk listings")
        systemid = {
            "Popular":"7",
            "Sky":"5",
            "Virgin M+":"2",
