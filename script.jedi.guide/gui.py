...
if ADDON.getSetting('skin.source') == "0":
    ...
    SKIN_PATH = xbmc.translatePath("special://profile/addon_data/script.jedi.guide/")
...
class TVGuide(xbmcgui.WindowXML):
    ...
    def __init__(self):
        ...
        f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/categories.ini','rb')
    ...
    def loadTVDBImages(self):
        file_name = 'special://profile/addon_data/script.jedi.guide/tvdb.pickle'
    ...
    def getControl(self, controlId):
        ...
        try:
            ...
            #xbmc.log("EXCEPTION: (script.jedi.guide) TVGuide.getControl %s" % detail, xbmc.LOGERROR)
    ...
    def close(self):
        if not self.isClosing:
            ...
            f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/tvdb.pickle','wb')
            try:
            ...
            file_name = 'special://profile/addon_data/script.jedi.guide/custom_stream_urls_autosave.ini'
    ...
    def play_catchup(self, program):
        file_name = 'special://profile/addon_data/script.jedi.guide/catchup.ini'
    ...
    def ChooseStreamAddon(self, result, channel):
        ...
        if url:
            if url.startswith('plugin://'):
                ...
                name = "url"
                icon = xbmcaddon.Addon('script.jedi.guide').getAddonInfo('icon')
        stream = ""
        title = ""
    ...
    def onActionEPGMode(self, action):
        ...
        if action.getId() in COMMAND_ACTIONS["NEXT_DAY"]:
            ...
            if results:
                if len(results) > 1:
                    ...
                    if what > -1:
                        ...
                        xbmcgui.Dialog().notification("Jedi Guide", "Couldn't find: %s" % title)
                else:
                    if selection == 0:
            ...
            if kodi < 16:
                dialog.ok('Jedi Guide', 'Editing categories in Kodi %s is currently not supported.' % kodi)
            else:
                ...
                f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/categories.ini','wb')
            ...
            xbmc.log('[script.jedi.guide] Unhandled ActionId: ' + str(action.getId()), xbmc.LOGDEBUG)
    ...
    def onActionQuickEPGMode(self, action):
        ...
        if action.getId() in COMMAND_ACTIONS["LEFT"]:
            ...
            xbmc.log('[script.jedi.guide] quick epg Unhandled ActionId: ' + str(action.getId()), xbmc.LOGDEBUG)
    ...
    def programSearch(self):
        ...
        file_name = "special://profile/addon_data/script.jedi.guide/title_search.list"
        f = xbmcvfs.File(file_name,"rb")
    ...
    def descriptionSearch(self):
        d = xbmcgui.Dialog()
        title = ''
        file_name = "special://profile/addon_data/script.jedi.guide/synopsis_search.list"
        f = xbmcvfs.File(file_name,"rb")
        searches = sorted(f.read().splitlines())
        f.close()
        actions = ["New Search", "Remove Search"] + searches
    ...
    def categorySearch(self):
        d = xbmcgui.Dialog()
        f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/category_count.ini',"rb")
    ...
    def _showContextMenu(self, program):
        ...
        if buttonClicked == PopupMenu.C_POPUP_REMIND:
            ...
            if ADDON.getSetting('search.type') == 'MySearch':
                script = "special://home/addons/script.jedi.guide/search.py"
    ...
    def clear_catchup(self):
        if not self.playing_catchup_channel:
            return
        self.playing_catchup_channel = False
        filename = 'special://profile/addon_data/script.jedi.guide/catchup_channel.list'
    ...
    def catchup_meta(self,channel):
        ...
        f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/catchup_channel.list','wb')
        first_cmd = None
    ...
    def catchup_direct(self,channel):
        ...
        now = datetime.datetime.now()
        offset = now - programList[0].startDate
        f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/catchup_channel.strm','wb')
        f.write("#EXTM3U\n")
        newProgramList = []
        for program in programList:
            program.startDate += offset
        ...
        if ADDON.getSetting('catchup.channel') == 'true':
            self.currentChannel = channel
            self.currentProgram = self.database.getCurrentProgram(self.currentChannel)
        xbmc.executebuiltin("PlayMedia(special://profile/addon_data/script.jedi.guide/catchup_channel.strm)")
    ...
    def playWithChannel(self, channel, program = None):
        ...
        if url:
            ...
            if ffmpeg:
                ...
                script = "special://home/addons/script.jedi.guide/playwithchannel.py"
                xbmc.executebuiltin('RunScript(%s,%s,%s)' % (script,channel.id,timestamp))

            script = "special://profile/addon_data/script.jedi.guide/playwithchannel.py"
            if xbmcvfs.exists(script):
                xbmc.executebuiltin('RunScript(%s,%s,%s)' % (script,channel.id,timestamp))
    ...
    def stopWith(self):
        ...
        if ffmpeg:
            folder = ADDON.getSetting('autoplaywiths.folder')
            script = "special://home/addons/script.jedi.guide/stopwithchannel.py"
            xbmc.executebuiltin('RunScript(%s)' % (script))

        script = "special://profile/addon_data/script.jedi.guide/stopwithchannel.py"
        if xbmcvfs.exists(script):
            xbmc.executebuiltin('RunScript(%s)' % (script))
        xbmc.Player().stop()
    ...
    def saveActions(self):
        file_name = 'special://profile/addon_data/script.jedi.guide/actions.json'
        f = xbmcvfs.File(file_name,"wb")
        ...
        if ADDON.getSetting('mapping.m3u.enabled') == 'true':
            ...
            if data:
                ...
                if encode and enckey:
                    ...
                    f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/mapping.aes.m3u','wb')
                    f.write(ddata)
                    f.close()
                    import_m3u = False
                elif enckey:
        ...
        if ADDON.getSetting('alt.mapping.tsv.enabled') == 'true':
            ...
            if data:
                ...
                if encode and enckey:
                    ...
                    ddata=encryptor.encrypt(data)
                    ddata=base64.b64encode(ddata)
                    f = xbmcvfs.File('special://profile/addon_data/script.jedi.guide/alt.mapping.aes.tsv','wb')
                    f.write(ddata)
                    f.close()
                    import_tsv = False
                elif enckey:
...
class StreamSetupDialog(xbmcgui.WindowXMLDialog):
        ...
        if controlId == self.C_STREAM_BROWSE_ADDONS:
            ...
            if stream:
                ...
                if len(lines) > 1:
                    ...
                    file_name = 'special://profile/addon_data/script.jedi.guide/addons.ini'
            ...
            if item:
                ...
                d = xbmcgui.Dialog()
                d.notification("Jedi Guide", title, sound=False, time=500)
    ...
    def updateDirsInfo(self):
        file_name = 'special://profile/addon_data/script.jedi.guide/folders.list'
        f = xbmcvfs.File(file_name)
        lines = f.read().splitlines()
        folders = []
        for folder in lines:
    ...
    def addBrowseFolder(self):
        ...
        file_name = 'special://profile/addon_data/script.jedi.guide/icons.ini'
