...
class Service(object):
    ...
    def onInit(self, success):
        if success:
            xbmc.log("[script.jedi.guide] Background Update Starting...", xbmc.LOGNOTICE)
    ...
    def onCachesUpdated(self):
        ...
        xbmc.log("[script.jedi.guide] Background Update Finished", xbmc.LOGNOTICE)
        if ADDON.getSetting('background.notify') == 'true':
            d = xbmcgui.Dialog()
            d.notification("Jedi Guide", "Finished Updating")
if __name__ == '__main__':
    ADDON = xbmcaddon.Addon('script.jedi.guide')

    version = ADDON.getAddonInfo('version')
    if ADDON.getSetting('version') != version:
        #text = xbmcvfs.File('special://home/addons/script.jedi.guide/changelog.txt','rb').read()
        #xbmcgui.Dialog().textviewer("Jedi Guide",text)
    ...
    xbmcvfs.delete('special://profile/addon_data/script.jedi.guide/source.db-journal')
    lock = 'special://profile/addon_data/script.jedi.guide/db.lock'
    xbmcvfs.delete(lock)
    try:
        if ADDON.getSetting('autostart') == "true":
            xbmc.executebuiltin("RunAddon(script.jedi.guide)")

        if ADDON.getSetting('background.service') == 'true':
            monitor = xbmc.Monitor()
            xbmc.log("[script.jedi.guide] Background service started...", xbmc.LOGDEBUG)
            if ADDON.getSetting('background.startup') == 'true':
                ...
                if ADDON.getSetting('service.addon.folders') == "true":
                    xbmc.executebuiltin('RunScript(special://home/addons/script.jedi.guide/ReloadAddonFolders.py)')
            while not monitor.abortRequested():
                if ADDON.getSetting('service.type') == '0':
                    ...
                    xbmc.log("[script.jedi.guide] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                else:
                ...
                if timeLeft < 0:
                    timeLeft = 0
                xbmc.log("[script.jedi.guide] Service waiting for %d seconds" % timeLeft, xbmc.LOGDEBUG)
                if timeLeft and monitor.waitForAbort(timeLeft):
                    break
                xbmc.log("[script.jedi.guide] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                if ADDON.getSetting('service.addon.folders') == "true":
                    xbmc.executebuiltin('RunScript(special://home/addons/script.jedi.guide/ReloadAddonFolders.py)')
                now = time.time()
        ...
        xbmc.log('[script.jedi.guide] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)
